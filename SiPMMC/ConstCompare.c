...
//Leakage is copied from ReadOutput.c
//currently have enough colours for 10 files
...
void ConstCompare(int len, TString files[len], long total_evts=0, TString var=""){
	...
	Style_t markers[10] = {kFullCrossX,kOpenSquare,kFullDiamond,kCircle,kFullTriangleUp,kStar,kFullCircle,kFullSquare,kFullTriangleDown,kOpenDiamond};
...
//for (int i=1;i<numbins+1;i++) cout << "energy: " << hLeakEnergy->GetBinCenter(i)-0.25 << " leakage: " << hLeakEnergy->GetBinContent(i) << endl;
	...
	hLeakEnergy->SetMarkerStyle(markers[0]);
	hLeakEnergy->SetMarkerColor(colors[0]);
	hLeakEnergy->GetXaxis()->SetTitle("Recoil energy (keV)");
	hLeakEnergy->GetYaxis()->SetTitle("Leakage");
	if (var=="PDE"){
		hLeakEnergy->SetTitle("Events: "+num+", Light coverage: "+name_coll_eff+", TPB: "+OnOff+", Type: "+evt_type+"R");
		leg1 = new TLegend(.65,.89-0.03*len,.89,.89);
		leg1->AddEntry(hLeakEnergy, "PDE: "+name_pde, "LP");
	}
	else if (var=="LC"){
		hLeakEnergy->SetTitle("Events: "+num+", PDE: "+name_pde+", TPB: "+OnOff+", Type: "+evt_type+"R");
		leg1 = new TLegend(.12, .12, .5, .5);
		leg1->AddEntry(hLeakEnergy, " LC: "+name_coll_eff, "LP");
	}
	else if (var==""){
	...
	fracErrs[0] = fracErr;
	zeros[0] = 0.0;
//cout << "PDE: " << pde << " LC: " << coll_eff << " fracBad: " << fracBad << endl;
	...
	for (int i=1;i<len;i++){
		...
		hLeakEnergy->SetMarkerStyle(markers[i]);
		hLeakEnergy->SetMarkerColor(colors[i]);
		if (evt_type=='E'){
			hLeakEnergy->Draw("E1same");
			if (var=="PDE") leg1->AddEntry(hLeakEnergy, "PDE: "+name_pde, "LP");
			else if (var=="LC") leg1->AddEntry(hLeakEnergy, " LC: "+name_coll_eff, "LP");
			leg1->Draw("same");
		}
...
//cout << "PDE: " << pde << " LC: " << coll_eff << " fracBad: " << fracBad << endl;
	...
	fracPDE->SetTitle("Events: "+num+", Light coverage: "+name_coll_eff+", TPB: "+OnOff+", Type: "+evt_type+"R");
	fracPDE->GetXaxis()->SetTitle("PDE");
	fracPDE->GetYaxis()->SetTitle("fraction of events with poorly reconstructed prompt fraction");
	...
	fracLC->GetXaxis()->SetTitle("Light coverage");
	fracLC->GetYaxis()->SetTitle("fraction of events with poorly reconstructed prompt fraction");
	fracLC->SetMarkerStyle(kFullCircle);
	fracLC->Draw("APE");
//for (int i=0;i<len;i++) cout << "coll_eff: " << coll_effs[i] << " frac: " << fracs[i] << endl;
