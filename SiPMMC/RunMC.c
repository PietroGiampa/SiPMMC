...
//Steps
// Loop through the events.  An event is a high-energy particle entering the detector.
	//a)  A random energy is generated between energy_min and energy_max.
	//b)  Use a gaussian (?)  prompt fraction distribution for NR and ER to randomly select a true prompt fraction based on the event energy.
	//c)  The number of collected photons is reduced by the light coverage.
	//d)  For each event, loop through the collected photons.
		//i.  Based on the measured LAr emission spectrum, randomly select a wavelength.
		//ii.  Using the prompt fraction, assign an emission time to the photon.
		//iii.  Blur the emission time with the SiPM resolution.
		//iv.  The number of detected photons is reduced by the photon detection efficiency.
	//e)  Add correlated dark pulses and dark noise to the event.
	//f)  Calculate the measured prompt fraction with all of the detector effects added in.
...
void RunMC(long evt_max, int IsTPBon, int seed, TString evt_type)
{
  ...
  TF1 *SiPMCDP = new TF1("SiPMCDP",SiPMcdpPDF,0,6000,6); //doesn't appear to be used
  ...
  TGraphErrors *gNRs = SingletToTripletSCENE("NR"); //isn't used
